...
**새 컴퓨터 세팅 순서:**
1. 현 컴퓨터에서 `./sync.sh` (GitHub에 최신 push)
2. 새 컴퓨터에 Git, Obsidian, Claude Code 설치
3. `git clone https://github.com/pgaey/LLM_Wiki.git`
4. Obsidian → Open folder as vault → 클론 폴더 선택

**일상 동기화:** 작업 시작·종료 시 `./sync.sh` 실행 (pull→commit→push 자동)
...
> 각 커맨드 상세 행동 순서: `.claude/commands/<커맨드명>.md` 참조.
